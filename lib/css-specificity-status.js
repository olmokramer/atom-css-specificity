...
import {addEventListener, isCSSTextEditor, isActiveTextEditor, highlightSelector, cssClassForSpecificityType} from './utils.js';
...
Object.assign(prototype, {
  ...
  buildText({specificity, selector, parts}) {
    [inline, ids, classes, elements] = specificity.split(',');
    var text = `Specificity:
      ${cssClassForSpecificityType('inline', inline)},
      ${cssClassForSpecificityType('ids', ids)},
      ${cssClassForSpecificityType('classes', classes)},
      ${cssClassForSpecificityType('elements', elements)}`;
    if(atom.config.get('css-specificity.showSelectorInStatusBar'))
      text = `Selector: ${highlightSelector(selector, parts)}, ${text}`;
    return text;
  },
